...
fn load_from_modules_dir() {
    ...
    let _marine = Marine::<WasmerBackend>::with_raw_config(raw_config).expect("Marine::<WasmerBackend> should load all modules");
}
...
fn wasi_mapped_dirs() {
    ...
    let raw_config = TomlMarineConfig::load(config_path).expect("Config must be loaded");
    let mut marine = Marine::<WasmerBackend>::with_raw_config(raw_config).expect("Marine::<WasmerBackend> should load all modules");
